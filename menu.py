...
def NewClient():
    ...
    def returntomenu ():
            newclientWindow.destroy()
            window.deiconify()

    #Main Window
    newclientWindow = tkr.Toplevel()
    newclientWindow.title("Registrar un Nuevo Cliente ") 
    newclientWindow.configure(bg='#333333')
    newclientmainframe = tkr.Frame(newclientWindow);
    newclientmainframe.configure(bg='#333333')
    newclientmainframe.pack()


    ##########################Client information #################################
    client_info_frame = tkr.LabelFrame(newclientmainframe,text="Información del Cliente")
    client_info_frame.grid(row=0,column=0,sticky="news", padx=20,pady=10)
    client_info_frame.configure(bg='#333333')

    first_name_label = tkr.Label(client_info_frame,text="Nombre: ")
    first_name_label.grid(row=0,column=0)
    first_name_entry = tkr.Entry(client_info_frame)
    first_name_entry.config(bg="#333333")
    ...
    last_name_entry.config(bg="#333333",fg="#FFFFF0")
    ...
    barber_information = tkr.LabelFrame(newclientmainframe,text="Información del Corte")
    barber_information.grid(row=1,column=0,sticky="news", padx=20,pady=10)
    barber_information.configure(bg='#333333')
    ...
    client_observations_frame = tkr.LabelFrame(newclientmainframe,text="Observaciones del Cliente")
    ...
    #####     BUTTONS     #####
    submtion_button = tkr.Button(newclientmainframe,text="Enviar Datos",command=submtion_data)
    submtion_button.grid(row=0,column=2,sticky="news",padx=20,pady=20)

    menu_button = tkr.Button(newclientmainframe,text="Menu Principal",command=returntomenu)
...
def mainmenu():
    ...
    newclientregister = tkr.Button(mainFrame, text= "Registar nuevo cliente",command=lambda: submition_button(window))
    newclientregister.grid(row=0,column=0,sticky="news",padx=20,pady=20);
    
    if barberimg is None:
    ...
    exitbutton = tkr.Button(mainFrame,text="Cerrar",command=endcode)
